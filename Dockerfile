...
RUN /bin/bash -c "source ~/.bashrc && pip install pymilvus==2.4.8 langchain langchain_community mistralai"

# Check if pymilvus is installed 
RUN /bin/bash -c "source ~/.bashrc && python -c 'import pymilvus'"

# Install sentence-transformers using pip
RUN /bin/bash -c "source ~/.bashrc && pip install transformers torch torchvision torchaudio requests beautifulsoup4 numpy"
...
ENV HUGGINGFACE_HUB_TOKEN="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"
